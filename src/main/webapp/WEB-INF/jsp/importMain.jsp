...
<script>
    //JavaScript代码区域
      ...
      //执行一个 table 实例
		table.render({
			...
			height: 523,
			//请求后台的地址
			url: '/ww.market/toSelectAllImport' //数据接口
				,
		    //表格的名称
			title: '进货信息表',
			//开启分页
    ...
    $("#add").click(function(){
		  layer.open({
			  ...
			  content:  '/ww.market/toinsertImportBill' ,/* '/ww.market/toinsertStaffBill',$("#window") */
	  ...
	  table.on('tool(test)', function(obj){ 
	    ...
	    if(layEvent === 'del'){
	      layer.confirm('真的删除行么', function(index){
	        layer.close(index);
	        //向服务端发送删除指令
	        var importid=data.imortbillid;
	        $.ajax({
	        	url:'/ww.market/toDelImport',
        		type:'post',
        		datatype:'json',
        		data:{
        			importid:importid
